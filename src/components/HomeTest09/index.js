...
import './style.css'
import { getUsers, getUser, createUsers, editUsers, deleteUsers } from '../../apis/users'
...
const HomeTest05 = () => {
    ...
    const validate = (list, formdata) => {
        ...
        if (formdata.id) {
            const item = list.find((item) => {
                return item.id !== formdata.id && (item.name === formdata.name || item.email === formdata.email)
            })
            return item ? false : true
        }
    }
    ...
    const onSubmit = () => {
        ...
        if (isValidate) {
            ...
            if (formdata.id) {
                editUsers(formdata.id, formdata).then(response => {
                    fetData()
                    setFormData(DEFAULT_FORM_DATA)
                }).catch(error => {
                    console.log(error)
    ...
    return (
        <div>
            <header className='header row'>
                ...
                <div className='col-4'>
                    <input className='form-control' placeholder='Type to search' value={search} onChange={onSearch}></input>
                </div>
                <div className='col-2'>
                    <button type='button' className='btn btn-primary border1' data-bs-toggle='modal' data-bs-target='#modal-form-user' >Create</button>
                </div>
            </header>
            {isloading && (
            ...
            <Form formData={formdata} onCreate={onCreate} onSearch={onSearch} onChange={onChange} onSubmit={onSubmit} />
            {list.length === 0 && (
